...
**Secrets:** no plaintext committed. `jukebox0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret jukebox0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `jukebox0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `jukebox0` | `sdk-apps-dev` | jukebox0-dev-topia.topia-rtsdk.com | `/api/system/health` |
